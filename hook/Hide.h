...
int HideFile(std::string& fileName)
{
	...
	w_path = std::wstring(path.begin(), path.end());
	w_file_name = std::wstring(file_name.begin(), file_name.end());
    /*for (size_t i = 0; i < sizeof(hook_functions) / sizeof(hook_functions[0]); i++)
    {
        //LPVOID func_original = GetProcAddress(GetModuleHandleA(dll_name.c_str()), function_names[i].c_str());
        FARPROC func_original = GetProcAddress(LoadLibraryA("kernel32.dll"), function_names[i].c_str());
        if (func_original == nullptr)
            return -1;
        DetourTransactionBegin();
        DetourUpdateThread(GetCurrentThread());
        DetourAttach(&(PVOID&)func_original, &(PVOID&)hook_functions[i]);
